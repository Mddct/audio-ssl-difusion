# copy from https://github.com/AmphionTeam/TaDiCodec/blob/main/models/tts/tadicodec/llama_nar_prefix.py
import math
import os
...
class LlamaAdaptiveRMSNorm(nn.Module):
    ...
    def forward(self,
                ...
                cond_embedding: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
...
class DiffLlamaPrefix(LlamaModel):
    ...
    def __init__(
        ...
        use_cond: bool = True,
        config: LlamaConfig = LlamaConfig(vocab_size=0,
                                          hidden_size=256,
    ...
    def forward(
        ...
        if self.use_diff_step and diffusion_step is not None:
            ...
            diffusion_step_emb = self.diff_step_embedding(diffusion_step).to(
                x.device)
            diffusion_step_emb = self.diff_step_mlp(
                diffusion_step_emb)  # (B, C)
        else:
